...
function launchGame() {
  ...
  outputTitle.textContent = "Shreya:";
  document.querySelector("#status").classList.add("add-alert");
  const samSpeech = [
    "Hi! Welcome to my house. I need your help. My brother has gone missing. Can you help me?",
    "I last saw him at Pizza Peel Restaurant. Apparently he went looking for berries.",
...
function goToMap() {
  ...
  outputContent.textContent = "Find Het. He was last seen at Pizza Peel.";
  if (newGame === null) {
    newGame = new Game();
...
function handleEndOfGame(event) {
  if (event.target.value.toLowerCase() == "unga unga") {
    ...
    const finalLocation = new Location(
      "island-power",
      [
        "Hey, Diduuuuu. Thank you so much for saving me!",
        "For some reason, I decided to go look for berries after decoration? I’m as confused as you are.",
        "I must’ve fallen asleep in the village. All I remember is waking up trapped on this island. ",
        "Anyway, let’s go back to Pizza Peel, it’s almost time for your party",
        "",
      ],
      "no-item",
      "Nima",
